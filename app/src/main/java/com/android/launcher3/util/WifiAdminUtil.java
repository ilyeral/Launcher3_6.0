...
public class WifiAdminUtil {
     ...
     * @return
     */
    public String getWifiType(String strSSID){
        startScan();
        for(int i=0;i<mWifiList.size();i++){
     ...
     * @return<br>
     */
    public boolean isConnect(Context context,ScanResult result) {
        if (result == null) {
            return false;
        }
        //获得当前已连接网络
        ...
        //获得网络信息
        android.net.NetworkInfo.State wifi = cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI)
                .getState();
    ...
    public boolean isWifiConnected(Context context,String SSID){
        ...
        while(wifi == android.net.NetworkInfo.State.CONNECTING ){
            try {
     ...
     * @return
     */
    public boolean isConnected(Context context,String SSID){
        //获得当前已连接的网络信息
